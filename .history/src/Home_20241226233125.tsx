...
function Home() {
  const [input, setInput] = useState<inputData>({name: '', password: ''});

  const handleInput = (e:any, source:string) => {
    if(source === 'user') //input for user field
      setInput(curr => ({
        ...curr,
        name: e.target.value,
      }));
  ...
  return (
    <div style={{display:'flex', justifyContent: 'center'}}>
      ...
      <LoginDiv>
        ...
        <StyledInput placeholder='username' value={input.name} onChange={(e) => handleInput(e, 'user')}/>
        <StyledInput placeholder='password' value={input.password} onChange={(e) => handleInput(e, 'password')}/>
