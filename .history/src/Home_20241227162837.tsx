  ...
  return (
    <div style={{display: 'flex', flexDirection: 'row'}}>
      ...
      <div style={{paddingLeft: 30, display: 'flex', alignItems: 'center', width: '85vw', flexDirection: 'column'}}>
        ...
        <h3 style={{margin: 10}}> Estimated Time: &nbsp; {jobInfo.remain_time} hrs</h3>
        ...
        <h3> Status: </h3> 
        <h3 style={{color: `${jobInfo.status===1? 'green' : jobInfo.status===2? 'yellow' : 'red'}`}}> {jobInfo.status===1 ? 'Success' : jobInfo.status===2 ? 'Working' : 'Failure'} </h3>
      </div>
    </div>
  )
}
