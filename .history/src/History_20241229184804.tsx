...
function History() {
    ...
    <div style={{display: 'flex', flexDirection: 'row'}}>
      ...
      <div style={{paddingLeft: 30, display: 'flex', justifyContent: 'center', alignItems: 'center', width: '85vw', flexDirection: 'column'}}>
        <table style={{width: '80%', borderRadius: '15px', boxShadow: `rgb(239, 36, 29) 10px 3px 8px;`}}>
          ...
          {
            data.map((d, i) => (
              <tr style={{textAlign: 'center', background: `${i%2==0? '#5c5959': '#a19f9f'}`}}>
                <td style={{padding: 10}}>{d.name}</td>
                <td style={{padding: 10, color: `${d.status===1? 'green' : d.status===2? 'yellow' : 'red'}`}}>
                  {d.status===1? 'success' : d.status===2? 'pending' : 'failure' }
                </td>
                <td style={{padding: 10}}>{d.time.toFixed(2)} hrs</td> 
                <td style={{padding: 10}}>{d.account}</td> 
              </tr>
            ))
          }
        </table>
