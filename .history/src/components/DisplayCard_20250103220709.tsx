...
const divCss = ` 
    ...
    .blink_sm_class{
        animation: blink_sm 3s ease-in-out infinite;
    }
    @keyframes blink_sm{
        25% {
...
const DisplayCard: FC<Props> = ({title, info, black_bg, status=true}) => {
    return (
        <div className={black_bg?'displayCard accent_avant':'displayCard classic_black'} style={{display: 'flex'}}>
            <style>{divCss}</style>
            <h5 style={{
            ...
            { status?
                <div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 10, flexGrow: 0}}>
                    <div className='big_circle blink_big_class'><div className='sm_circle blink_sm_class'></div></div>
                    <h3 style={{
