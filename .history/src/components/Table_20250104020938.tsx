...
const Table = () => {
    ...
    return (
        <div style={{marginTop: '5rem'}} className='table'>
            ...
            {tableData?.map(t => (
                <div className='trow'>
                    <h6 style={{width: '35%'}} >{t.job}</h6>
                    <h6 style={{width: '15%'}} className={t.status==='Working'?'Warning':t.status}>{t.status}</h6>
                    <h6 style={{width: '25%'}} >{t.time} hrs</h6>
                    <h6 style={{width: '25%'}} >{t.account}</h6>
                </div>
            ))
