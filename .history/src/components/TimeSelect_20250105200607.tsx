...
const TimeDial: FC<Props> = ({title}) => {
    ...
    return(
        <div style={{display: 'flex', flexFlow: 'row wrap', justifyContent: 'center', alignItems: 'center', textAlign: 'center', gap: 10}}>
            ...
            <h5>:</h5>
            <input style={inputStyle} type='number' min='00' max='59' onChange={(e) => hrsHandler(e)} />
            <div style={{backgroundColor: 'var(--accent-30)', padding: '.5rem', borderRadius: '.5rem', textAlign: 'center'}}>
                <h6 style={{backgroundColor: 'var(--accent-100)', borderRadius: '.5rem', padding: '.3rem .5rem', marginBottom: '.4rem'}}>AM</h6>
                <h6 style={{backgroundColor: 'var(--bg-100)', borderRadius: '.5rem', padding: '.3rem .5rem', }}>PM</h6>
            </div>
        </div>
    )
}
