...
const DisplayCard: FC<Props> = ({title='', info, black_bg, status=true, fn, fullWidth=false}) => {
    return (
        <div onClick={fn} className={black_bg?'displayCard accent_avant':'displayCard classic_black'} style={fullWidth?{display: 'flex', flexDirection: 'column', width: '95%'}:{display: 'flex', flexDirection: 'column'}}>
            <style>{divCss}</style>
            <h5 style={{
