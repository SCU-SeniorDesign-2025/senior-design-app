...
const TimeSelect = () => {
    ...
    const handleSelect = (d: string) => {
        if(!selectedDays.includes(d)){
            console.log('day to add: ', d);
            setSelectedDays(s => [...s, d]);
        }
    }
  return (
    <div style={{backgroundColor: 'var(--bg-89)', borderRadius: '.5rem', padding: '3rem 1rem', marginTop: '2rem', marginBottom: '-2rem'}}>
        <style>{`
            .sel_day{
        ...
        <div style={{display: 'flex', flexDirection: 'row', gap: '.75rem', justifyContent: 'center', marginBottom: '2rem'}}>
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thr', 'Fri', 'Sat'].map(day => (
                ...
                style={{borderRadius: '50%', backgroundColor: 'var(--accent-30)', padding: '1rem'}}
