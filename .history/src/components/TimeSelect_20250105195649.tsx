...
const TimeDial: FC<Props> = ({title}) => {
    ...
    return(
        <div style={{display: 'flex', flexFlow: 'row wrap', justifyContent: 'center', alignItems: 'center', textAlign: 'center', gap: 10}}>
            ...
            <h5>:</h5>
            <input type='number' min='1' max='12' onChange={(e) => hrsHandler(e)} />
            <div style={{backgroundColor: 'var(--accent-30)', padding: '.5rem', borderRadius: '.5rem', textAlign: 'center', gap: '.7rem'}}>
                <h4 style={{backgroundColor: 'var(--accent-100)', borderRadius: '.5rem', }}>AM</h4>
                <h4 style={{backgroundColor: 'var(--accent-100)', borderRadius: '.5rem', }}>PM</h4>
            </div>
        </div>
    )
}
