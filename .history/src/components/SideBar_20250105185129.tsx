...
const SideBar: FC<Props> = ({currPage}) => {
    ...
    return (
        <div style={{
            ...
            <FontAwesomeIcon className={currPage==='create'?'iconStyle current': 'iconStyle'} onClick={() => nav('/create')} icon={faPlus} />
            <FontAwesomeIcon className={currPage==='jobs'?'iconStyle current': 'iconStyle'} onClick={() => nav('/jobs')} icon={faChartSimple} />
        </div>
    )
}
